...
static void aml_new_usb_otg_notifier_call(unsigned long is_device_on)
{
...
void aml_new_otg_init(void)
{
	...
	if (r5.b.iddig_curr == 0) {
		...
		aml_new_usb_otg_notifier_call(0);
...
static int amlogic_new_otg_init(struct amlogic_otg *phy)
{
	...
	for (i = 0; i < 6; i++) {
		if (!usb_otg_aml_regs.usb_r_v2[i])
			return 0;
...
static void amlogic_gxl_work(struct work_struct *work)
{
	...
	if (r5.b.iddig_curr == 0) {
		...
		aml_new_usb_otg_notifier_call(1);
