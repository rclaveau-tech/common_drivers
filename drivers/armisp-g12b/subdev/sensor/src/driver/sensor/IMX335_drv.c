...
static void sensor_alloc_integration_time(void *ctx, uint16_t *int_time_S, uint16_t *int_time_M, uint16_t *int_time_L)
{
    ...
    case WDR_MODE_LINEAR: // Normal mode
        if (*int_time_S > p_ctx->vmax_adjust - 1)
            *int_time_S = p_ctx->vmax_adjust - 1;
        if (*int_time_S < 9)
            *int_time_S = 9;
    ...
    case WDR_MODE_FS_LIN: // DOL2 Frames
        if (*int_time_S < 10)
            *int_time_S = 10;
        if (*int_time_S > p_ctx->max_S)
            *int_time_S = p_ctx->max_S;
        if (*int_time_L < 120)
            *int_time_L = 120;

        if (*int_time_L > p_ctx->max_L)
            *int_time_L = p_ctx->max_L;
