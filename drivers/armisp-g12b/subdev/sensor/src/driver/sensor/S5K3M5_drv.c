...
static const char p_sensor_data[] = SENSOR__S5K3M5_SEQUENCE_DEFAULT;
static const char p_isp_data[] = SENSOR__S5K3M5_ISP_SEQUENCE_DEFAULT;
#else
    ...
    if ( p_ctx->int_cnt || p_ctx->gain_cnt ) {
        ...
        if ( p_ctx->gain_cnt ) {
            ...
            case WDR_MODE_LINEAR:
                acamera_sbus_write_u16( p_sbus, 0x0204, (p_ctx->again[p_ctx->again_delay] ) & 0xFFFF );
                break;
        ...
        if ( p_ctx->int_cnt ) {
            switch ( p_ctx->wdr_mode ) {
            case WDR_MODE_LINEAR:
                acamera_sbus_write_u16( p_sbus, 0x0202, ( p_ctx->int_time_S ) & 0xFFFF );
                break;
            case WDR_MODE_FS_LIN:
                break;
            }
...
*/
    acamera_sbus_write_u16(&p_ctx->sbus, 0xFCFC,0x4000);
    sensor_id = acamera_sbus_read_u16(&p_ctx->sbus, 0x0000);
    if (sensor_id != SENSOR_CHIP_ID) {
        LOG(LOG_CRIT, "%s: Failed to read sensor read is 0x%x != 0x%x\n", __func__,sensor_id,SENSOR_CHIP_ID);
        return 0xFFFF;
    }
...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
    ...
    if ( (param->modes_table[mode].exposures == 1) && (param->modes_table[mode].fps == 30 * 256) ) {
        acamera_sbus_write_u16( p_sbus, 0x3840, 0x0cf2 );// s5k3m5
        p_ctx->s_fps = 30;
        p_ctx->vmax = 3306; // s5k3m5 3314-8
    }  else {
        p_ctx->vmax = ((uint32_t)acamera_sbus_read_u16(p_sbus,0x3840)) - 8;
    }

    param->total.width = (uint16_t)acamera_sbus_read_u16( p_sbus, 0x034c );//s5k3m5 line-length-pix
    param->lines_per_second = p_ctx->pixel_clock / param->total.width;
    param->total.height = (uint16_t)acamera_sbus_read_u16( p_sbus, 0x034e );//s5k3m5 frame-length-lines
...
static void stop_streaming( void *ctx )
{
    ...
    acamera_sbus_write_u16(p_sbus, 0x0100, 0x0000);
...
static void start_streaming( void *ctx )
{
    ...
    acamera_sbus_write_u16(p_sbus, 0x0100, 0x0100);
}
...
static void sensor_deinit_s5k3m5( void *ctx )
{
...
static sensor_context_t *sensor_global_parameter(void* sbp)
{
    ...
    ret = pwren_am_enable(sensor_bp,"pwren", 1);
    if (ret < 0 )
        pr_err("set pwren fail\n");

    sensor_ctx.sbp = sbp;

    sensor_ctx.sbus.mask = SBUS_MASK_SAMPLE_16BITS | SBUS_MASK_ADDR_16BITS | SBUS_MASK_SAMPLE_SWAP_BYTES | SBUS_MASK_ADDR_SWAP_BYTES;
    ...
    sensor_ctx.again_limit = 0x200;
    sensor_ctx.pixel_clock = 482000000;
    ...
    sensor_ctx.param.isp_context_seq.seq_num = SENSOR_S5K3M5_ISP_CONTEXT_SEQ;
...
//--------------------Initialization------------------------------------------------------------
static void sensor_init_s5k3m5( void **ctx, sensor_control_t *ctrl, void *sbp )
{
...
static int sensor_detect_s5k3m5( void* sbp)
{
    int ret = 0;
    sensor_ctx.sbp = sbp;
    sensor_bringup_t* sensor_bp = (sensor_bringup_t*) sbp;
    ...
    //sensor_ctx.sbus.mask = SBUS_MASK_SAMPLE_16BITS | SBUS_MASK_ADDR_16BITS | SBUS_MASK_ADDR_SWAP_BYTES;
    sensor_ctx.sbus.mask = SBUS_MASK_SAMPLE_16BITS | SBUS_MASK_ADDR_16BITS | SBUS_MASK_SAMPLE_SWAP_BYTES | SBUS_MASK_ADDR_SWAP_BYTES;
    ...
    pr_info("sensor_detect_s5k3m5:%d\n", ret);
...
EXPORT_SYMBOL(sensor_init_s5k3m5);
EXPORT_SYMBOL(sensor_deinit_s5k3m5);
EXPORT_SYMBOL(sensor_detect_s5k3m5);
