...
#include "SC8238CS_seq.h"
#include "SC8238CS_config.h"
...
static sensor_mode_t supported_modes[] = {
    {
        ...
        .bayer = BAYER_BGGR,
        .dol_type = DOL_NON,
        .num = SENSOR_SC8238CS_SEQUENCE_1920_1080_30FPS_10BIT_4LANE_BINNING,
    },
    {
        .wdr_mode = WDR_MODE_LINEAR,
        .fps = 25 * 256,
        ...
        .bayer = BAYER_BGGR,
        .dol_type = DOL_NON,
        .num = SENSOR_SC8238CS_SEQUENCE_3840_2160_30FPS_10BIT_4LANE,
    },
    {
        .wdr_mode = WDR_MODE_LINEAR,
        .fps = 25 * 256,
    ...
    },
    {
        ...
        .bayer = BAYER_BGGR,
        .dol_type = DOL_VC,
        .num = SENSOR_SC8238CS_SEQUENCE_1920_1080_30FPS_10BIT_4LANE_WDR_BINNING,
    },
    {
        .wdr_mode = WDR_MODE_FS_LIN,
        .fps = 25 * 256,
    ...
    },
    {
        ...
        .bayer = BAYER_BGGR,
        .dol_type = DOL_VC,
        .num = SENSOR_SC8238CS_SEQUENCE_3840_2160_15FPS_10BIT_4LANE_WDR,
    },
};

//-------------------------------------------------------------------------------------
#if SENSOR_BINARY_SEQUENCE
static const char p_sensor_data[] = SENSOR_SC8238CS_SEQUENCE_DEFAULT;
static const char p_isp_data[] = SENSOR_SC8238CS_ISP_CONTEXT_SEQUENCE;
#else
static const acam_reg_t **p_sensor_data = sc8238cs_seq_table;
    ...
    if ( p_ctx->int_cnt || p_ctx->gain_cnt ) {
        ...
        if ( p_ctx->int_cnt ) {
            ...
            case WDR_MODE_LINEAR:
                ...
                break;
            case WDR_MODE_FS_LIN:
                p_ctx->int_time_L = ( p_ctx->int_time_L >> 2 ) << 2;
                p_ctx->int_time_S = ( p_ctx->int_time_S >> 2 ) << 2;
                if ( p_ctx->int_time_S < 3) p_ctx->int_time_S = 3;
...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
    ...
    case WDR_MODE_FS_LIN:
        ...
        break;
    default:
        LOG(LOG_ERR, "Invalid wdr mode. Returning!");
        return;
    }
    ...
    if ( param->active.width == 1920 ) {
        if ( (param->modes_table[mode].exposures == 1 ) && (param->modes_table[mode].fps == 25 * 256) ) {
            acamera_sbus_write_u8( p_sbus, 0x320e, 0x0a ); //0x0752 = 1874
            acamera_sbus_write_u8( p_sbus, 0x320f, 0x8c );
            p_ctx->s_fps = 25;
            p_ctx->vmax = 2700 ;
        ...
        if ( (param->modes_table[mode].exposures == 1 ) && (param->modes_table[mode].fps == 25 * 256) ) {
            ...
            rhs1 = ((uint32_t)acamera_sbus_read_u8(p_sbus,0x3e23)<<8) |acamera_sbus_read_u8(p_sbus,0x3e24);
            p_ctx->max_S = 2 * (rhs1 - 8) ;
            p_ctx->max_L = 2 * ( p_ctx->vmax - rhs1 - 9 );
        } else {
            p_ctx->vmax = ((uint32_t)acamera_sbus_read_u8(p_sbus,0x320e)<<8)|acamera_sbus_read_u8(p_sbus,0x320f);
            p_ctx->max_L = 2 * p_ctx->vmax - 10 ;
        }
    ...
    // pixel_clock = HTS * VTS * FPS; SC8238CS: HTS = HTS * 2;
    if ( param->active.width == 1920 ) {
        if ( param->modes_table[mode].exposures == 2 ) {
            p_ctx->pixel_clock = 178200000;
        } else {
            p_ctx->pixel_clock = 280800000;
        }
    } else {
        p_ctx->pixel_clock = 280800000;
    }
...
static void sensor_deinit_sc8238cs( void *ctx )
{
    sensor_context_t *t_ctx = ctx;
...
static sensor_context_t *sensor_global_parameter(void* sbp)
{
    ...
    sensor_ctx.again_limit = 8063;
    sensor_ctx.pixel_clock = 280800000;
    ...
    sensor_ctx.param.isp_context_seq.seq_num= SENSOR_SC8238CS_CONTEXT_SEQ;
...
//--------------------Initialization------------------------------------------------------------
static void sensor_init_sc8238cs( void **ctx, sensor_control_t *ctrl, void* sbp)
{
...
static int sensor_detect_sc8238cs( void* sbp)
{
    int ret = 0;
    sensor_ctx.sbp = sbp;
    sensor_bringup_t* sensor_bp = (sensor_bringup_t*) sbp;
...
EXPORT_SYMBOL(sensor_init_sc8238cs);
EXPORT_SYMBOL(sensor_deinit_sc8238cs);
EXPORT_SYMBOL(sensor_detect_sc8238cs);
